...
class Update(mixins.MonthCalendarMixin, UpdateView):
    ...
    def get(self, request, **kwargs):
        if WorkTime.objects.get(id=self.kwargs["pk"]).name != request.user:
            return HttpResponse("不正なアクセスです。")
        return super().get(request)
...
class WorkIndex(ListView):
    ...
    def get_queryset(self):
        query_set = WorkTime.objects.filter(name=self.request.user).order_by("-date")
    ...
    def get_context_data(self, **kwargs):
        ...
        context["objects"] = WorkTime.objects.filter(name=self.request.user).order_by(
            "-date"
        )
