...
from work_time.models import WorkTime
...
def time_count(start, end):
    ...
    if end_time >= night_time:
        ...
        work_time = [
            [int(str(time).split(":")[0]), int(str(time).split(":")[1])]
...
def work_time(object):
    ...
    if end_time >= night_time or end_time <= dt.time(hour=5):
        work_time = time_count(start_time, end_time)[0]
    else:
        work_time = time_count(start_time, end_time)
...
def work_time(object_list):
    ...
    time_sum = (
        str(time_hour_sum + time_minutes_sum // 60)
        + "時間"
        + str(time_minutes_sum % 60)
        + "分"
    )
    night_sum = (
        str(night_hour_sum + night_minutes_sum // 60)
        + "時間"
        + str(night_minutes_sum % 60)
        + "分"
    )
    over_sum = (
        str(over_hour_sum + over_minutes_sum // 60)
        + "時間"
        + str(over_minutes_sum % 60)
        + "分"
    )
